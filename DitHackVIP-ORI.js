...
(function () {
  ...
  async function validateLicense(licenseCode) {
    try {
      ...
      if (expiryDate < now) {
        return {
          valid: false,
          message: "Token sudah expired pada " + license.expired,
        };
      }

      return {
        valid: true,
        message: "Selamat datang " + license.owner + "!",
        owner: license.owner,
      };
    } catch (error) {
  ...
  function createLicenseModal() {
    ...
    infoText.innerHTML =
      'Belum punya token? <a href="https://dithackvip.vercel.app" target="_blank" style="color: #4b4bfF; text-decoration: none;">Beli di sini</a>';
  ...
  async function fetchAnswers(pin, resultsEl) {
    try {
      if (resultsEl) {
        resultsEl.innerHTML =
          '<div style="text-align:center; color:#666; padding:20px;">⏳ Menghubungi server...</div>';
      }

      const response = await fetch(
        `https://api.quizit.online/quizizz?pin=${encodeURIComponent(pin)}`,
        {
          method: "GET",
          headers: {
      ...
      if (
        !data ||
        (data.data && !data.data.answers) ||
        (data.data && data.data.answers && data.data.answers.length === 0) ||
  ...
  async function fetchKahootAnswers(link, resultsEl) {
    try {
      ...
      const response = await fetch(
        `https://api.quizit.online/kahoot?link=${encodeURIComponent(link)}`,
        {
          method: "GET",
          headers: {
            Accept: "application/json",
  ...
  function getAnswersArray() {
    if (cachedData && cachedData.data && cachedData.data.answers)
      return cachedData.data.answers;
  ...
  function renderCards(answers, searchTerm = "") {
    ...
    if (searchTerm) {
      ...
      filteredAnswers = answers.filter((ans) => {
        const questionText =
          ans.question &&
          (ans.question.text ||
            (ans.question.structure && ans.question.structure.text))
            ? (ans.question.text || ans.question.structure.text).replace(
                /<[^>]*>/g,
                "",
              )
            : "";
        const answerText =
          ans.answers && ans.answers.map((a) => a.text).join(" ")
            ? ans.answers
                .map((a) => a.text)
                .join(" ")
                .replace(/<[^>]*>/g, "")
    ...
    filteredAnswers.forEach((ans, index) => {
      ...
      if (
        ans.question &&
        ans.question.image &&
        typeof ans.question.image === "string"
      ) {
      ...
      const answerImageBorderStyle =
        "max-width:100%; border-radius:8px; margin-top:4px; border: 3px solid #22c55e;";
    ...
    setTimeout(() => {
      document
        .querySelectorAll(".math-question, .math-answer")
        .forEach((el) => {
  ...
  function renderKahootCards(answers, searchTerm = "") {
    ...
    if (searchTerm) {
      ...
      filteredAnswers = answers.filter((ans) => {
        const questionText =
          ans.question && ans.question.text
            ? ans.question.text.toLowerCase()
            : "";
        const answerText =
          ans.answers && ans.answers[0] && ans.answers[0].text
            ? ans.answers[0].text.toLowerCase()
            : "";
